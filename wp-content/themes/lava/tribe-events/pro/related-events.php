...
<div class="events-single__related">
	<h2 class="tribe-events-related-events-title"><?php printf( esc_html__( 'Related %s', 'lava' ), tribe_get_event_label_plural() ); ?></h2>
	...
	<ul class="tribe-related-events tribe-clearfix">
		...
		<li class="equal-height">
				...
				<a href="<?php echo esc_url( tribe_get_event_link( $post ) ); ?>" class="url" rel="bookmark" tabindex="-1">
					<?php
						if ( has_post_thumbnail( $post->ID ) ) {
							get_the_post_thumbnail( $post->ID, 'lava_thumb_small' );
						} else {
							echo '<img src="' . esc_url( trailingslashit( Tribe__Events__Pro__Main::instance()->pluginUrl ) . 'src/resources/images/tribe-related-events-placeholder.png' ) . '" alt="' . esc_attr( get_the_title( $post->ID ) ) . '" />';
						}
					?>
				</a>
			</div>
			<div class="tribe-related-event-info">
				...
				<div class="events-single-related-meta">
				<?php
