...
<?php do_action( 'hotel_booking_before_checkout_form' ); ?>
    ...
    <div id="hotel-booking-payment">
        ...
        <form name="hb-payment-form" id="hb-payment-form" method="post" action="<?php echo isset( $search_page ) ? $search_page : ''; ?>">
            <h3><?php esc_html_e( 'Booking Rooms', 'lava' ); ?></h3>
            <div class="hb-booking-details">
                <table class="hb_table">
                    ...
                    <tr class="hb_sub_total">
                        <td colspan="7"><?php esc_html_e( 'Sub Total', 'lava' ); ?>
                            ...
                            <tr class="hb_payment_all">
                                <td colspan="7" class="hb-align-right">
                                    <label class="hb-align-right">
                                        <input type="checkbox" name="pay_all" id="hb_wt_payment_all" class="lava-checkbox"/>
        								<?php esc_html_e( 'I want to pay all', 'lava' ); ?>
                                    </label>
                                </td>
                            </tr>
        				<?php } ?>
			...
			<?php if ( !is_user_logged_in() && !hb_settings()->get( 'guest_checkout' ) ) : ?>
				<?php printf(
                        wp_kses( 
                            __( 'You have to <strong><a href="%s">login</a></strong> or <strong><a href="%s">register</a></strong> to checkout.', 'lava' ), 
                            array(
                                'a' => array(
                                    'href' => array(),
                                ),
			...
			<?php else : ?>
                ...
                <div class="hb-checkout-payment">
                    ...
                    <div class="hb-checkout-row">
                        <?php if ( $tos_page_id = hb_get_page_id( 'terms' ) ) { ?>
                            <p class="hb_payment_agreement">
                                <label>
                                    <input type="checkbox" name="tos" id="tos-agreement" value="1" class="lava-checkbox"/><?php printf( esc_html__( 'I agree with ', 'lava' ) . '<a href="%s" target="_blank">%s</a>', get_permalink( $tos_page_id ), get_the_title( $tos_page_id ) ); ?>
                                </label>
                            </p>
        				<?php } ?>
                        <p>
                            <button type="submit" class="hb_button btn-primary"><?php esc_html_e( 'Check out', 'lava' ); ?></button>
                        </p>
                    </div>
                </div>
			<?php endif; ?>
