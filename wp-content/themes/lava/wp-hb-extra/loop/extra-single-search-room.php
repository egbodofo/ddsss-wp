					...
					<li data-price="<?php echo esc_attr( $extra->amount_singular ); ?>">
						<div class="hb_extra_optional_right">
							<input type="checkbox"
								...
								class="hb_optional_quantity_selected lava-checkbox" id="<?php echo esc_attr( 'hb-ex-room-' . $post->id . '-' .$key ) ?>"
							/>
						</div>
							...
							<div class="hb_extra_detail_price">
								<?php if ( $extra->respondent === 'number' ): ?>
									<input type="number" step="1" min="1" name="hb_optional_quantity[<?php echo esc_attr( $extra->ID  ); ?>]" value="1"
										class="hb_optional_quantity<?php if ( $extra->respondent !== 'number' ) { echo ' tp_hb_readonly'; } ?>"
										<?php if ( $extra->respondent !== 'number' ) { echo ' readonly="readonly"'; } ?>
									/>
								<?php else: ?>
