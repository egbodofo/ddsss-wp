...
<tr class="hb_checkout_item package" data-cart-id="<?php echo esc_attr( $cart_id ) ?>" data-parent-id="<?php echo esc_attr( $package->parent_id ) ?>">
	...
	<td colspan="<?php echo is_hb_cart() ? 2 : 1 ?>">
		<?php if ( is_hb_cart() && !get_post_meta( $package->product_id, 'tp_hb_extra_room_required' ) ): ?>
			<a href="#" class="hb_package_remove" data-cart-id="<?php echo esc_attr( $cart_id ) ?>" data-parent-id="<?php echo esc_attr( $package->parent_id ) ?>"><i class="material-icons">close</i></a>
		<?php endif; ?>
	</td>
	...
	<td>
		<?php if ( is_hb_cart() ) : ?>
			<?php if ( $input = apply_filters( 'hb_extra_cart_input', $package->product_data->respondent ) ): ?>
				<input type="number" min="1" value="<?php echo esc_attr( $package->quantity ); ?>" name="hotel_booking_cart[<?php echo esc_attr( $cart_id ); ?>]" class="input-field-small"/>
			<?php else: ?>
				<?php printf( '%s', $package->quantity ) ?>
				<input type="hidden" value="<?php echo esc_attr( $package->quantity ); ?>" name="hotel_booking_cart[<?php echo esc_attr( $cart_id ); ?>]" class="input-field-small"/>
			<?php endif; ?>
		<?php else: ?>
			<?php printf( '%s', $package->quantity ) ?>
		<?php endif; ?>
