...
function business_store_general_admin_notice(){
	...
	if($pagenow == 'index.php' || $pagenow == 'themes.php'){
         ...
         $msg = sprintf('<div data-dismissible="disable-done-notice-forever" class="notice notice-info is-dismissible" >
			 	...
			 	<a href="?business_store_notice_dismissed" target="_self"  style="text-decoration: none; margin-left:10px;float:right" >%5$s</a>
			 	<strong>%6$s</strong></p></div>',				
				esc_url(business_store_THEME_DOC),
				esc_html__('Documentation','business-store'),
				esc_url(business_store_THEME_URL),	
				esc_html__('Learn More','business-store'),
...
if ( isset( $_GET['business_store_notice_dismissed'] ) ){
	update_option('business_store_adm_notice', -1);
}
$business_store_adm_notice = get_option('business_store_adm_notice', 1);
if($business_store_adm_notice > -1){
