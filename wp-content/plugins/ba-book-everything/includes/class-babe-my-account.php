  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title" id="exampleModalLabel">'.__( 'Thank you for registration! Check email for your password.', BABE_TEXTDOMAIN ).'</h3>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-footer justify-content-center">
        <button type="button" class="btn btn-primary" data-dismiss="modal">'.__( 'Close', BABE_TEXTDOMAIN ).'</button>
      </div>
    </div>
  </div>
</div>
...
//////////////////////////////    
    ...
    public static function get_register_form(){
       ...
       <div id="login_registration">
        ...
        <h3>'.__('Do not have an account?', BABE_TEXTDOMAIN).'</h3>
        
        <!-- Button trigger modal -->
        <div class="registration_link"><a href="#registration" data-toggle="modal" data-target="#registration">'.__('Create an account', BABE_TEXTDOMAIN).'</a></div>
        <!-- -->
...
<div class="modal fade" id="registration" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h2 class="modal-title">'.__('Create an account', BABE_TEXTDOMAIN).'</h2>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      ...
      <div class="modal-body">
        ...
        <form name="registration_form" id="registration_form" action="'.BABE_Settings::get_my_account_page_url(array('action' => 'registration')).'" method="post">
            ...
            <div class="new-username">
				<label for="new_username">'.__('Username*', BABE_TEXTDOMAIN).'</label>
            ...
            <div class="new-username-check">
              <span class="new-username-check-msg">'.__('This username already exists', BABE_TEXTDOMAIN).'</span>
            </div>
            
            <div class="new-first-name">
				<label for="new_first_name">'.__('First name*', BABE_TEXTDOMAIN).'</label>
				<input type="text" name="new_first_name" id="new_first_name" class="input" value="" size="20" required="required">
			</div>
            <div class="new-last-name">
				<label for="new_last_name">'.__('Last name*', BABE_TEXTDOMAIN).'</label>
				<input type="text" name="new_last_name" id="new_last_name" class="input" value="" size="20" required="required">
			</div>
            
            <div class="new-email">
				<label for="new_email">'.__('Your email*', BABE_TEXTDOMAIN).'</label>
				<input type="text" name="new_email" id="new_email" class="input" value="" size="20" required="required">
                <div class="new-email-check-msg">'.__('This email already exists', BABE_TEXTDOMAIN).'</div>
			</div>
            <div class="new-email">
				<label for="new_email_confirm">'.__('Confirm email*', BABE_TEXTDOMAIN).'</label>
				<input type="text" name="new_email_confirm" id="new_email_confirm" class="input" value="" size="20" required="required">
			</div>
            			
            <div class="statement">
               <span class="register-notes">'.__('A password will be emailed to you.', BABE_TEXTDOMAIN).'</span>
            </div>
			
			<div class="new-submit">
				<input type="submit" name="new-submit" id="new-submit" class="button button-primary" value="'.__('Sign up', BABE_TEXTDOMAIN).'">
...
////////////////////////////        
    ...
    if (!is_user_logged_in()){
        ...
        //  'redirect'       => admin_url(),
          ...
          'id_submit'      => 'wp-submit',
          'label_username' => __( 'Your email:', BABE_TEXTDOMAIN ),
          'label_password' => __( 'Password:', BABE_TEXTDOMAIN ),
          'label_remember' => __( 'Remeber Me', BABE_TEXTDOMAIN ),
          'label_log_in'   => __( 'LOGIN', BABE_TEXTDOMAIN ),    
          'value_username' => '',
        ...
        */
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'login_error'){
            $message .= '
          <div id="login_error">
            '.__( '<strong>ERROR</strong>: Invalid username or password.', BABE_TEXTDOMAIN ).' <a href="'.BABE_Settings::get_my_account_page_url(array('action' => 'lostpassword')).'">'.__( 'Forgot your password?', BABE_TEXTDOMAIN ).'</a>
          </div>';
        }
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'password_reseted'){
          ...
          <div id="password_reseted">
            '.__( 'Check your email address for you new password.', BABE_TEXTDOMAIN ).'
          </div>';
        }
        ...
        if (!isset($_GET['action'])){
            $message .= '
          <div id="forgot_url">
            <a href="'.BABE_Settings::get_my_account_page_url(array('action' => 'lostpassword')).'">'.__( 'Forgot password?', BABE_TEXTDOMAIN ).'</a>
          </div>';
        }
        ...
        <div id="login_form">
          ...
          <h3>'.__('Login', BABE_TEXTDOMAIN).'</h3>
          ...
          <form name="babe_login" id="babe_login" action="'.BABE_Settings::get_my_account_page_url(array('action' => 'login')).'" method="post">
			...
			<div class="login_username">
				<label for="login_username">'.__('Username or email', BABE_TEXTDOMAIN).'</label>
				<input type="text" name="login_username" id="login_username" class="input" value="" size="20" required="required">
			</div>
			<div class="login_pw">
				<label for="login_pw">'.__('Password', BABE_TEXTDOMAIN).'</label>
            ...
            <div class="login_submit">
				<input type="submit" name="login_submit" id="login_submit" class="button button-primary" value="'.__('Sign in', BABE_TEXTDOMAIN).'">
			</div>
...
////////////////////////////        
    ...
    if (!is_user_logged_in()){
        ...
        <h2>'.__( 'Reset password', BABE_TEXTDOMAIN ).'</h2>
        ...
        <form id="lostpassword_reset" name="lostpassword_reset" method="post" action="">
            ...
            <div class="lost_username">
				<label for="user_email">'.__( 'Your email', BABE_TEXTDOMAIN ).'</label>
            ...
            <div class="lost_submit">
               <button class="btn button lostpassword_submit">'.__('Get new password', BABE_TEXTDOMAIN).'</button>
            </div>
...
///////////////////////////////////////
  ...
  public static function get_nav_arr( $check_role ){
    ...
    if ($check_role == 'customer'){
        $output = array(
         'dashboard' => __('Dashboard', BABE_TEXTDOMAIN),
         'profile' => array(
            'title' => __('My Profile', BABE_TEXTDOMAIN),
            'edit-profile' => __('Edit Profile', BABE_TEXTDOMAIN),
            'change-password' => __('Change Password', BABE_TEXTDOMAIN),
         ),
         'activity' => array(
            'title' => __('Activity', BABE_TEXTDOMAIN),
            'my-bookings' => __('My Bookings', BABE_TEXTDOMAIN),
         ),
         'logout' => __('Sign Out', BABE_TEXTDOMAIN),
       );
    }
    ...
    if ($check_role == 'manager'){
        $output = array(
         'dashboard' => __('Dashboard', BABE_TEXTDOMAIN),
         'activity' => array(
            'title' => __('Reservations', BABE_TEXTDOMAIN),
            'my-orders' => __('Orders', BABE_TEXTDOMAIN),
         ),
       );
       
       $post_type_arr = array(BABE_Post_types::$booking_obj_post_type, BABE_Post_types::$service_post_type, BABE_Post_types::$faq_post_type);
       ...
       $output['logout'] = __('Sign Out', BABE_TEXTDOMAIN);
...
///////////////////////////////////////
  ...
  public static function customer_dashboard( $content, $user_info ){
    ...
    if ( !isset($_GET[self::$account_page_var]) || $_GET[self::$account_page_var] == 'dashboard'){
        ...
        $output .= self::get_my_bookings_html(__('My Bookings', BABE_TEXTDOMAIN), $user_info);
  ...
  public static function get_customer_edit_profile_html($user_info){
    ...
    $input_fields[] = '
            <div class="edit-profile-form-block edit-profile-avatar">
              ...
              <a class="btn button button_link" href="https://gravatar.com" target="_blank">'.__('Change Profile Picture', BABE_TEXTDOMAIN).'</a>
            </div>';
    ...
    ///////// fields
      ...
      foreach($args as $field_name => $field_content){
          ...
          */
            ...
            <div class="new-email-check-msg">'.__('This email already exists', BABE_TEXTDOMAIN).'</div>' : $check_msg;    
    ...
    <div class="my_account_inner_page_block my_account_edit_user_profile">
       <h2>'.__('Edit Profile', BABE_TEXTDOMAIN).'</h2>
       <form id="edit_user_profile" name="edit_user_profile" method="post" action="">
            ...
            <div class="submit_group">
               <button class="btn button edit_user_profile_submit">'.__('Update profile', BABE_TEXTDOMAIN).'</button>
               <div class="form-spinner"><i class="fas fa-spinner fa-spin fa-2x"></i></div>
            </div>
       </form>
    </div>';
...
///////////////////////////////////////
  ...
  public static function get_change_user_password_html(){
    ...
    <div class="my_account_inner_page_block my_account_change_user_password">
       <h2>'.__('Change password', BABE_TEXTDOMAIN).'</h2>
       <form id="change_user_password" name="change_user_password" method="post" action="">
            
            <div class="user_profile_fields_group input_group">
            
            <div class="edit-profile-form-block">
               <label class="edit_profile_form_input_label">'.__('Old Password *', BABE_TEXTDOMAIN).'</label>
               <div class="edit_profile_form_input_field">
            ...
            <div class="edit-profile-form-block">
               <label class="edit_profile_form_input_label">'.__('New Password *', BABE_TEXTDOMAIN).'</label>
               <div class="edit_profile_form_input_field">
            ...
            <div class="edit-profile-form-block">
               <label class="edit_profile_form_input_label">'.__('Confirm Password *', BABE_TEXTDOMAIN).'</label>
               <div class="edit_profile_form_input_field">
            ...
            <div class="submit_group">
               <button class="btn button change_user_password_submit">'.__('Update password', BABE_TEXTDOMAIN).'</button>
            </div>
       </form>
    </div>';
...
///////////////////////////////////////
  ...
  public static function manager_dashboard( $content, $user_info ){
    ...
    if ( !isset($_GET[self::$account_page_var]) || $_GET[self::$account_page_var] == 'dashboard' ){
        ...
        $output .= self::get_my_bookings_html(__('Orders', BABE_TEXTDOMAIN), $user_info);
...
/////////////////////
    ...
    $head['order_num']   = __('Order #', BABE_TEXTDOMAIN);
    $head['items']  = __('Items', BABE_TEXTDOMAIN);          
    $head['date_from']    = __('Date from', BABE_TEXTDOMAIN);
    $head['date_to']   = __('Date to', BABE_TEXTDOMAIN);
    $head['guests']   = __('Guests', BABE_TEXTDOMAIN);
    $head['total_amount'] = __('Total amount', BABE_TEXTDOMAIN);
    // $head['prepaid_amount'] = __('Prepaid amount', BABE_TEXTDOMAIN);
    $head['status'] = __('Status', BABE_TEXTDOMAIN);
    //$head['prepaid_received'] = __('Prepaid received', BABE_TEXTDOMAIN);
...
///////////////////////////////////
    ...
    if ($column_name == 'total_amount') {
       $output .= BABE_Currency::get_currency_price(BABE_Order::get_order_total_amount($post_id));
    }
    
    if ($column_name == 'prepaid_amount') {
       $output .= BABE_Currency::get_currency_price(BABE_Order::get_order_prepaid_amount($post_id));
    }
    
    if ($column_name == 'prepaid_received') {
       $output .= BABE_Currency::get_currency_price(BABE_Order::get_order_prepaid_received($post_id));
    }
    
    if ($column_name == 'status') {
        $status = BABE_Order::get_order_status($post_id);
      $output .= '<div class="my_account_my_bookings_order_status order_status_'.$status.'">'.BABE_Order::$order_statuses[$status].'</div>';
      $check_role = self::validate_role($user_info); 
      
      if ($check_role == 'customer' && $status == 'payment_expected'){
          $output .= '<a class="my_bookings_table_a_button btn button button_link btn-paynow" href="'.BABE_Order::get_order_payment_page($post_id).'">'.__('Pay Now', BABE_TEXTDOMAIN).'</a>';
      }
      ...
      if ($check_role == 'manager' && $status == 'av_confirmation'){
          ...
          $output .= '<a class="my_bookings_table_icon_button btn button icon-button-confirm" href="#" title="'.__('Confirm', BABE_TEXTDOMAIN).'"><i class="fas fa-check-square"></i></a>
          <a class="my_bookings_table_icon_button btn button icon-button-reject" href="#" title="'.__('Reject', BABE_TEXTDOMAIN).'"><i class="fas fa-window-close"></i></a>';
        
          $output .= '<a class="my_bookings_table_a_button btn button button-disabled button_link btn-av-confirm" href="'.BABE_Settings::get_my_account_page_url($current_args).'">'.__('Confirm', BABE_TEXTDOMAIN).'</a>
          ';
          
          $current_args['action_value'] = 'reject';
          
          $output .= '
          <a class="my_bookings_table_a_button btn button button-disabled button_link btn-av-reject" href="'.BABE_Settings::get_my_account_page_url($current_args).'">'.__('Reject', BABE_TEXTDOMAIN).'</a>';
      }
...
///////////////////////////////////////
  ...
  public static function new_post(){
    ...
    if (is_singular() && $post->ID == (int)BABE_Settings::$settings['my_account_page'] && isset($_GET[self::$account_page_var]) && isset($post_type_arr[$_GET[self::$account_page_var]])){
        $user_info = wp_get_current_user();
        if ($user_info->ID > 0){
         $check_role = self::validate_role($user_info);
         if ($check_role == 'manager'){
...
///////////////////////////////////////
  ...
  public static function get_all_posts_html($post_type, $user_info){
    ...
    while ( $the_query->have_posts() ) : $the_query->the_post();
       ...
       $output .= '<tr><td class="my_account_all_posts_td my_account_all_posts_td_title"><a href="'.$edit_url.'" title="'.__( 'Edit', BABE_TEXTDOMAIN ).'">'.get_the_title($post_id).'</a></td></tr>';
    ...
    if ($output){
        $output = '
    <div class="my_account_all_posts_total">'.__( 'Total: ', BABE_TEXTDOMAIN ).$found_posts.'</div>    
