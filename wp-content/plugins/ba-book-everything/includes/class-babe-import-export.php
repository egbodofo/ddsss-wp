...
class BABE_Import_export {
	...
	static $babe_import = false;

	static $shortcodes_to_watch = [];
	static $posts_with_shortcodes = [];

	static $redux_options_name = 'bahotel_settings';

	// mappings from old post data to new
	static $processed_terms = [];
	static $processed_posts = [];
    static $processed_attachments = [];
	static $processed_comments = [];
	static $redux_data = '';
	static $rates_data = '';
    static $rates_meta_data = '';
	static $avcal_data = '';
    static $booking_rules_data = '';
    static $discount_data = '';
	 ...
	 *
	 */
	static function delete_temp_import_posts() {
		...
		foreach ( $post_list as $item ) {
			...
			if ( !empty($find_post) && isset($find_post->post->ID) ){
     ...
     *
     */
    static function init_delete_temp_import_posts() {
	 ...
	 *
	 */
	static function init_shortcodes_array(){
	...
	public static function create_import_settings() {
		...
		add_settings_section(
			...
			__('Export/import', BABE_TEXTDOMAIN), // Title
		...
		add_settings_field(
			...
			__('Extend WP export/import with BA Book Everything data', BABE_TEXTDOMAIN), // Title
	 ...
	 *
	 */
	static function save_price_and_avcal(){
        ...
        $rates_meta = $wpdb->get_results("SELECT * FROM " . BABE_Prices::$table_rate_meta);

        $args = array(
            'meta_query' => array(
                array(
                    'key' => 'data_type',
                    'value' => 'babe_rates_meta_slug',
                    'compare' => '=',
                )
            )
        );
        ...
        if ( !empty($find_rates_post->posts) ){
            ...
            $args = array();
            $args['ID'] = $rates_post_id;
            $args['post_content'] = serialize($rates_meta);
            wp_update_post( wp_slash($args) );
        } else {
            $post_data = array(
                'post_title'    => 'babe_rates_meta_slug',
                'post_content'  => serialize($rates_meta),
                'post_status'   => 'draft',
                'post_author'   => 1,
                'post_type'     => 'post',
            );
        ...
        update_post_meta($rates_post_id, 'data_type', 'babe_rates_meta_slug');
		...
		$redux_options = get_option( self::$redux_options_name );
		...
		if (!empty($redux_options)){
			$args = array(
				'meta_query' => array(
					array(
						'key' => 'data_type',
						'value' => 'babe_'.self::$redux_options_name,
						'compare' => '=',
					)
				)
			);
			...
			if (!empty ($find_redux_post->posts) ){
				...
				$post_data = array(
					'post_title'    => 'babe_'.self::$redux_options_name,
			...
			update_post_meta($redux_post_id, 'data_type', 'babe_'.self::$redux_options_name);
		}
	 ...
	 * @param array $posts
	 */
	static function is_extended_export_active( $posts = [] ){
	 ...
	 * @return array $posts
	 */
	static function validate_posts_before_import($posts){

		self::is_extended_export_active($posts);

		if ( self::$babe_import && !empty($posts) ){
			foreach ($posts as $post_id => $post){
				if ( $post["post_title"] === 'babe_'.self::$redux_options_name ){
					self::$redux_data = maybe_unserialize($post["post_content"]);
					unset ( $posts[$post_id] );
				} elseif ( $post["post_title"] === 'babe_rates_slug' ){
                    self::$rates_data = maybe_unserialize($post["post_content"]);
                    unset ( $posts[$post_id] );
                } elseif ( $post["post_title"] === 'babe_rates_meta_slug' ){
                    self::$rates_meta_data = maybe_unserialize($post["post_content"]);
                    unset ( $posts[$post_id] );
	 ...
	 *
	 */
	static function save_rate_settings(){
    ...
    /**
     * Save rates meta to database.
     *
     */
    static function save_rates_meta(){

        if ( empty(self::$rates_meta_data) ){
            return;
        }

        global $wpdb;

        $count = $wpdb->get_var("SELECT count(id) FROM ". BABE_Prices::$table_rate_meta);

        if (!empty($count)){
            $wpdb->query( "DELETE FROM ".BABE_Prices::$table_rate_meta );
        }

        foreach ( self::$rates_meta_data as $rate_meta ) {
            $wpdb->insert(
                BABE_Prices::$table_rate_meta,
                (array)$rate_meta
            );
        }
     ...
     *
     */
    static function save_booking_rules(){
	 ...
	 * @return int
	 */
	static function save_avcal_settings(){
	 ...
	 * @return array
	 */
	static function map_comment( $comments, $post_id, $post){
	 ...
	 *
	 */
	static function map_imported_comment($key, $comment, $comment_post_ID, $post){
	 ...
	 *
	 */
	static function correct_imported_data(){
		...
		foreach ( self::$posts_with_shortcodes as $post_old_id => $post_arr ){
		    ...
		    if ( empty(self::$processed_posts[$post_old_id]) || get_post_meta( self::$processed_posts[$post_old_id], 'data_corrected', 1 ) ){
                continue;
            }
	 ...
	 * @return string
	 */
	static function correct_shortcodes_data( $post_arr ){
	 ...
	 * @return array
	 */
	static function extended_explode($string){

		$first_try = explode(', ', $string);
		if (count($first_try) === 1){
	 ...
	 * @return int
	 */
	static function get_contact7_form(){
	 ...
	 * @return string
	 */
	static function get_attachment_url_by_slug( $slug ) {
	 ...
	 *
	 */
	static function save_redux_settings() {

		self::correct_redux_data();

        update_option( self::$redux_options_name, self::$redux_data);
	 ...
	 *
	 */
	static function correct_redux_data(){

		self::$redux_data = maybe_unserialize(self::$redux_data);

		if ( empty(self::$redux_data) ){
		    return;
        }

		foreach ( self::$redux_data as $setting_id => $parameters ){
		    ...
		    if ( empty(self::$processed_attachments[$parameters['id']]) || $setting_id == 'logo' || $setting_id == 'footer_logo' ){
                self::$redux_data[$setting_id]['url'] = '';
                self::$redux_data[$setting_id]['thumbnail'] = '';
                self::$redux_data[$setting_id]['id'] = '';
                self::$redux_data[$setting_id]['title'] = '';
                self::$redux_data[$setting_id]['height'] = '';
                self::$redux_data[$setting_id]['width'] = '';
                continue;
            }

            $img_thumbnail_url = '';

		    if ( self::$redux_data[$setting_id]['url'] != self::$redux_data[$setting_id]['thumbnail'] ){
            ...
            self::$redux_data[$setting_id]['url'] = $img_full_src[0];
            self::$redux_data[$setting_id]['thumbnail'] = $img_thumbnail_url ?: $img_full_src[0];

            $image_meta = wp_get_attachment_metadata( $parameters['id'] );

            if ( isset( $image_meta['height'], $image_meta['width'] ) ) {
                self::$redux_data[$setting_id]['height'] = $image_meta['height'];
                self::$redux_data[$setting_id]['width'] = $image_meta['width'];
            }
		}
	 ...
	 * @return array
	 */
	static function post_parser($post){
	...
	public static function import_taxonomy_first() {
		...
		foreach ($terms as $id => $term){
			...
			if ($term["term_taxonomy"] === 'taxonomies_list'){
