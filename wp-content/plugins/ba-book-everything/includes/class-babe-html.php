...
//////////////////////////////
    ...
    public static function wp_enqueue_scripts() {
     ...
     global $post;
        ...
        ////////validation js////////
        wp_enqueue_script( 'babe-validate-js', plugins_url( "js/jquery.validate.min.js", BABE_PLUGIN ), array('babe-google-api'), BABE_VERSION, true );
     ...
     if ( BABE_DEV ){
        ...
        wp_enqueue_script( 'babe-js', plugins_url( "js/babe-scripts.js", BABE_PLUGIN ), array('babe-google-api'), BABE_VERSION, true );
        
     } else {
        
        wp_enqueue_script( 'babe-js', plugins_url( "js/babe-scripts.min.js", BABE_PLUGIN ), array('babe-google-api'), BABE_VERSION, true );
     ...
     ///// add google API
     wp_enqueue_script( 'babe-google-api', "https://maps.googleapis.com/maps/api/js?key=".BABE_Settings::$google_api_key."&libraries=places", array('jquery'), BABE_VERSION, true );
    ...
    $local_validate_langs = array('ar' => 'ar', 'az' => 'az', 'bg' => 'bg', 'ca' => 'ca', 'cs' => 'cs', 'da' => 'da', 'de' => 'de', 'el' => 'el', 'es' => 'es', 'et' => 'et', 'eu' => 'eu', 'fa' => 'fa', 'fi' => 'fi', 'fr' => 'fr', 'ge' => 'ge', 'gl' => 'gl', 'he' => 'he', 'hr' => 'hr', 'hu' => 'hu', 'hy' => 'hy', 'id' => 'id', 'is' => 'is', 'it' => 'it', 'ja' => 'ja', 'ka' => 'ka', 'kk' => 'kk', 'ko' => 'ko', 'lt' => 'lt', 'lv' => 'lv', 'mk' => 'mk', 'my' => 'my', 'nl' => 'nl', 'no' => 'no', 'pl' => 'pl', 'ro' => 'ro', 'ru' => 'ru', 'sd' => 'sd', 'si' => 'si', 'sk' => 'sk', 'sl' => 'sl', 'sr' => 'sr', 'sv' => 'sv', 'th' => 'th', 'tj' => 'tj', 'tr' => 'tr', 'uk' => 'uk', 'ur' => 'ur', 'vi' => 'vi', 'zh' => 'zh');
     
     if( isset($local_validate_langs[$lang])){
     wp_enqueue_script( 'babe-validate-local-js', plugins_url( "js/localization/messages_".$lang.".min.js", BABE_PLUGIN ), array('jquery'), BABE_VERSION, true );
     }
     ///////////////
     ...
     ///////////////////////////////////////
            ...
            $basic_booking_period = isset($rules_cat['rules']['basic_booking_period']) ? $rules_cat['rules']['basic_booking_period'] : '';
            $cal_first_click = $basic_booking_period && $basic_booking_period != 'recurrent_custom' && $basic_booking_period != 'single_custom';
     ...
     $travel_mode_html = '
               <div id="travel_mode_panel_modal">
                <label for="travel_mode_modal">'.__('Mode of Travel: ', BABE_TEXTDOMAIN).'</label>
                <select id="travel_mode_modal" name="travel_mode_modal">
                <option value="WALKING">'.__('Walking', BABE_TEXTDOMAIN).'</option>
                <option value="DRIVING">'.__('Driving', BABE_TEXTDOMAIN).'</option>
                <option value="BICYCLING">'.__('Bicycling', BABE_TEXTDOMAIN).'</option>
                </select>
               </div>
               
                <input class="address-autocomplete" name="autocomplete" placeholder="'.__( 'Enter your address', BABE_TEXTDOMAIN ).'" type="text" />
                ';
     ...
     wp_enqueue_style('jquery-ui-style', plugins_url( "css/jquery-ui.min.css", BABE_PLUGIN ), '', '1.12.1', 'all');
     ///
     wp_enqueue_style('babe-fontawesome', plugins_url( "fonts/fontawesome-free/css/all.min.css", BABE_PLUGIN ), '', '5.11.2', 'all');
         ...
         foreach ($order_items_arr as $order){
             ...
             if ( count($ages_arr) > 1 ){
                 foreach ( $ages_arr as $age ){
                     $age_dropdown .= '<li class="term_item" data-value="' . $age['description'] . '" data-id="' . $age['age_id'] . '" >' . $age['description'] . '</li>';
                 }
             }
             ...
             for ($i = 2; $i <= $total_number_of_guests; $i++){
                 $output .= '<h4>'. sprintf(__( 'Extra guest %s', BABE_TEXTDOMAIN ), $i) .'</h4>';
                 ...
                 $output .= '<div class="checkout-form-block">
				           <div class="checkout_form_input_field">
				               <label class="checkout_form_input_label">' . __( 'First name', BABE_TEXTDOMAIN ) . '</label>
							   <input type="text" class="checkout_input_field" name="extra_guests[' . $i .'][first_name]" id="guest' . $i .'_first_name" value=""'. $required . '>
				               <div class="checkout_form_input_underline"><span class="checkout_form_input_ripple"></span></div>
						   </div>
				        </div>
				        <div class="checkout-form-block">
				           <div class="checkout_form_input_field">
				               <label class="checkout_form_input_label">' . __( 'Last name', BABE_TEXTDOMAIN ) . '</label>
							   <input type="text" class="checkout_input_field" name="extra_guests[' . $i .'][last_name]" id="guest' . $i .'_last_name" value=""'. $required . '>
				               <div class="checkout_form_input_underline"><span class="checkout_form_input_ripple"></span></div>
						   </div>
				        </div>
				        ';
                 ...
                 if ($age_dropdown){
                     ...
                     $output .= '<div class="select_guests_block input_select_field">
							<div class="input_select_title add_ids_title">
                                 ...
                                 <label class="checkout_form_input_label" for="age_list">' . __( 'Age group', BABE_TEXTDOMAIN ) . '</label>
                                    <input type="text" id="age_group' . $i .'" class="input_select_input age_group" name="extra_guests_title[' . $i .'][age_group]" value=""'. $required . '>
                                    <input type="hidden" class="input_select_input_value" name="extra_guests[' . $i .'][age_group]" value="">
                                    <div class="checkout_form_input_underline"><span class="checkout_form_input_ripple"></span></div>
    ...
    public static function post_content($content){
        ...
        if (is_single() && in_the_loop() && is_main_query()){
          ...
          if ($post->post_type == BABE_Post_types::$mpoints_post_type){            
            ...
            $address_arr['address'] = get_post_meta($post->ID, 'address', 1);
...
////////////////////////////
    ...
    public static function babe_post_content_item_code($content, $post_id, $post){
        ...
        $content .= isset($post['code']) && $post['code'] ? '<div class="item_code">'.__( 'Item Code:', BABE_TEXTDOMAIN ).' <strong><span itemprop="productID">'.$post['code'].'</span></strong></div>' : '';
...
////////////////////////////
    ...
    public static function babe_post_content_event_date($content, $post_id, $post){
        ...
        if ($rules_cat['rules']['basic_booking_period'] == 'single_custom'){
            ...
            $content .= '<div class="single_event_dates">'.__( 'When:', BABE_TEXTDOMAIN ).' <span class="single_event_dates_value">'.$dates.'</span></div>';
...
////////////////////////////
    ...
    public static function babe_post_content_tabs($content, $post_id, $post){
        ...
        $tab_titles[] = __( 'Overview', BABE_TEXTDOMAIN );
        ...
        if(!BABE_Settings::$settings['av_calendar_remove'] && $rules_cat['rules']['basic_booking_period'] != 'single_custom'){
            ...
            $tab_titles[] = __( 'Calendar & Prices', BABE_TEXTDOMAIN );
            $tab_content[] = self::block_calendar($post);
            $tab_slugs[] = 'calendar';
        ...
        if ($block_steps){
            $tab_titles[] = __( 'Details', BABE_TEXTDOMAIN );
            $tab_content[] = $block_steps;
            $tab_slugs[] = 'steps';
            $tab_fa_classes[] = 'fas fa-info-circle';
        }

        if(!BABE_Settings::$settings['google_map_remove'] && isset($rules_cat['category_meta']['categories_gmap_active']) && $rules_cat['category_meta']['categories_gmap_active'] ){

            $block_meeting_points = self::block_meeting_points($post);
            $block_meeting_points_title = __( 'Meeting points', BABE_TEXTDOMAIN );
            if ( !$block_meeting_points ){
                if (isset($post['address']['address']) && isset($post['address']['latitude']) && isset($post['address']['longitude']) && $post['address']['latitude'] && $post['address']['longitude']){
                    ...
                    $block_meeting_points_title = __( 'Map', BABE_TEXTDOMAIN );
                } else {
        ...
        $services_title = isset($rules_cat['category_meta']['categories_services_title']) && !empty($rules_cat['category_meta']['categories_services_title']) ? $rules_cat['category_meta']['categories_services_title'] : __( 'Services', BABE_TEXTDOMAIN );
        ...
        $faq_title = isset($rules_cat['category_meta']['categories_faq_title']) && !empty($rules_cat['category_meta']['categories_faq_title']) ? $rules_cat['category_meta']['categories_faq_title'] : __( 'Questions & Answers', BABE_TEXTDOMAIN );
        ...
        if ( !empty(BABE_Settings::$settings['reviews_in_tabs']) ){
            ...
            if ( $show_reviews ){
                ...
                $tab_titles[] = __( 'Reviews', BABE_TEXTDOMAIN );
...
////////////////////////////
    ...
    public static function babe_post_content_block_related($content, $post_id, $post){
        ...
        $block_related_title = apply_filters( 'babe_post_content_related_title', __( 'You may like', BABE_TEXTDOMAIN ));
    ...
    public static function block_price_from($post){
        ...
        if (!empty($prices)){
            ...
            if ($prices['discount_price_from'] < $prices['price_from']){
                ...
                $save = '<span class="item_info_price_from_discount">'.BABE_Currency::get_currency_price($prices['price_from']).'</span><span class="item_info_price_from_save">'.__( 'Save ', BABE_TEXTDOMAIN ).$prices['discount'].'%</span>
            <div class="item_info_discount_expired"><i class="far fa-clock"></i> '.__( 'Offer Ends: ', BABE_TEXTDOMAIN ).$date_to_obj->format(get_option('date_format')).'</div>
            ';
            }

            $output .= '<div class="item_info_price">
       <label>'.__( 'From ', BABE_TEXTDOMAIN ).BABE_Currency::get_currency().'</label>
...
///////////////////////////
    ...
    public static function get_post_schema($post){
        ...
        if (!empty($prices)){
            ...
            $description = get_post_meta( $post['ID'], '_yoast_wpseo_metadesc', 1 );
            ...
            if ( $total_rating && $post['comment_count'] ){
                ...
                foreach ( $comments as $comment ) {
                    ...
                    $schema['review'][] = [
                        ...
                        'name' => esc_attr__('Worth a purchase', BABE_TEXTDOMAIN),
                        'reviewRating' => [
                            '@type' => 'Rating',
...
///search_result_description_price////       
    /**
				...
				<div class="babe_all_items_item_inner">
					...
					<div class="item_text related_item_text">
						...
						<div class="item_info_price">
							<label>' . __( 'from', BABE_TEXTDOMAIN ) . '</label>
							' . $price_old . '
...
/////ajax_get_meeting_points////    
    /**
	 * Get post meeting points as formated string.
     * @return string
	 */
    public static function ajax_get_meeting_points() {
        ...
        if (isset($_POST['post_id']) && BABE_Post_types::is_post_booking_obj($_POST['post_id']) && isset($_POST['lat']) && isset($_POST['lng']) && isset($_POST['nonce']) && wp_verify_nonce( $_POST['nonce'], self::$nonce_title)){
          ...
          if (!empty($post) && isset($post['meeting_points']) && isset($post['meeting_place']) && $post['meeting_place'] == 'point'){
           ...
           if (!empty($meeting_points)){
              foreach($meeting_points as $point_id => $meeting_point){      
                ...
                <button class="btn button add_destination" data-lat="'.$meeting_point['lat'].'" data-lng="'.$meeting_point['lng'].'" data-address="'.$meeting_point['address'].'" data-point-id="'.$point_id.'">'.__('Select address', BABE_TEXTDOMAIN).'</button>
              </div>';
              }
              ...
              $output .= '<h4>'.__('Select meeting point', BABE_TEXTDOMAIN).'<a href="#booking_form">'.__('Go to Booking form', BABE_TEXTDOMAIN).'</a></h4>'.implode('', $results);
...
/////ajax_booking_calculate////    
    ...
    public static function ajax_booking_calculate() {
        ...
        if (isset($_POST['post_id']) && BABE_Post_types::is_post_booking_obj($_POST['post_id']) && isset($_POST['date_from']) && isset($_POST['date_to']) && isset($_POST['nonce']) && wp_verify_nonce( $_POST['nonce'], self::$nonce_title)){
          ...
          $guests = isset($data_arr['booking_guests']) && !empty($data_arr['booking_guests']) && is_array($data_arr['booking_guests']) ? array_map('absint', $data_arr['booking_guests']) : [ 0 => 1 ];
          $services = isset($data_arr['booking_services']) && is_array($data_arr['booking_services']) ? array_map('absint', $data_arr['booking_services']) : [];

          $fees = isset($data_arr['booking_fees']) && is_array($data_arr['booking_fees']) ? array_map('absint', $data_arr['booking_fees']) : [];
          $mandatory_fees = BABE_Post_types::get_post_mandatory_fee_ids($babe_post);
          if ( !empty($mandatory_fees) ){
              $fees = array_merge($fees, $mandatory_fees);
          }
          ...
          $date_to = BABE_Calendar_functions::isValidDate($_POST['date_to'], BABE_Settings::$settings['date_format']) ? BABE_Calendar_functions::date_to_sql($_POST['date_to']) : $date_from; /// now in Y-m-d format
          $time_from = isset($_POST['time_from']) && BABE_Calendar_functions::isValidTime($_POST['time_from'], 'H:i') ? ' '.$_POST['time_from'] : '';
          $time_to = isset($_POST['time_to']) && BABE_Calendar_functions::isValidTime($_POST['time_to'], 'H:i') ? ' '.$_POST['time_to'] : ' ';
          
          $date_from = $date_from.$time_from;
          $date_to = $date_to.$time_to;
          
          if ($date_from){
             $price_arr = BABE_Prices::get_obj_total_price_arr($post_id, $date_from, $guests, $date_to, $services, $fees);
             $price = BABE_Prices::get_obj_total_price($post_id, $price_arr);
             $output = BABE_Currency::get_currency_price($price['total_with_taxes']);
    ...
    public static function ajax_get_times_guests() {
        ...
        if (isset($_POST['post_id']) && BABE_Post_types::is_post_booking_obj($_POST['post_id']) && isset($_POST['date_from']) && isset($_POST['date_to'])&& isset($_POST['nonce']) && wp_verify_nonce( $_POST['nonce'], self::$nonce_title)){
          ...
          if ($date_from){
            ...
            $guests = isset( $_POST['guests'] ) && !empty( $_POST['guests'] ) && is_array( $_POST['guests'] ) ? array_map('absint', $_POST['guests']) : array();
...
/////ajax_get_booking_times////    
    ...
    public static function ajax_get_booking_times() {
        ...
        if (isset($_POST['post_id']) && BABE_Post_types::is_post_booking_obj($_POST['post_id']) && isset($_POST['date_from']) && isset($_POST['start_time']) && isset($_POST['from_to']) && isset($_POST['nonce']) && wp_verify_nonce( $_POST['nonce'], self::$nonce_title)){
          ...
          if ($date_from){
            ...
            if ($rules_cat['rules']['basic_booking_period'] == 'day'){
                ...
                $output = self::input_select_field($field_name, '', $time_select_arr, $time_selected);

            } else if ($rules_cat['rules']['basic_booking_period'] === 'hour'){

            	if ( $from_to  === 'to') {
		            $time_selected_obj = new DateTime($start_time);
		            $time_selected_obj->modify('+1 hour');
	            }
	            $field_name = $from_to === 'from' ? 'booking_time_from' : 'booking_time_to';

	            $time_select_arr = self::get_time_select_arr($date_from, $post_id, ($from_to === 'from'), ( $from_to === 'to' && !empty($start_time) ? $start_time : '00:00'), [], 60, true);
	            $output = self::input_select_cloud($field_name, $time_select_arr);
            }
          }
...
////////////////////////////
     ...
     * @return string
	 */
    public static function booking_form_av_times($post_id, $date_from, $order_item_args = array()){
        
        $output = array();
        
        $date_from_obj = new DateTime($date_from);
        $av_cal = BABE_Calendar_functions::get_av_cal($post_id, $date_from_obj->format('Y-m-d'), $date_from_obj->format('Y-m-d'), $order_item_args);
        ...
        if (!empty($av_cal) && isset($av_cal[$date_from_obj->format('Y-m-d')]['times'])){
            ...
            foreach( $times as $time => $av_guests){
              ...
              if ( $av_guests ){  
                ...
                if ((!$i && !isset($times[$test_time])) || (isset($times[$test_time]) && $test_time == $time)){
...
////////////////////////////
    ...
    public static function booking_form_select_guests($post_id, $max_guests = 0, $date_from = '', $date_to ='', $selected_guests_arr = array()){
        ...
        $max_guests = !$max_guests ? absint( get_post_meta($post_id, 'guests', 1) ) : absint($max_guests);
        ...
        if ($max_guests && $date_from){
            ...
            if ( $rules_cat['rules']['basic_booking_period'] == 'recurrent_custom' ){
                ...
                $guests_min = $av_cal_first_rec['min_booking_period'] ? $av_cal_first_rec['min_booking_period'] : $guests_min;
            ...
            foreach ($post_ages as $age_term){
                ...
                if ( $rules_cat['rules']['basic_booking_period'] == 'recurrent_custom' || $rules_cat['rules']['booking_mode'] == 'tickets' || $rules_cat['rules']['booking_mode'] == 'places' ){
                    ...
                    if ($price_max_total_with_taxes != $price_total_with_taxes){
                ...
                $selected_value = isset($selected_guests_arr[$age_term['age_id']]) ? $selected_guests_arr[$age_term['age_id']] : 0;
						...
						<div class="select_guests_block input_select_field">
							<div class="input_select_title">
                                 ...
                                 <div class="input_select_wrapper">
                                    <input type="text" id="guests_'.$age_term['age_id'].'" class="input_select_input input_select_input_value select_guests" name="booking_guests['.$age_term['age_id'].']" data-age-id="'.$age_term['age_id'].'" value="'.$selected_value.'">
                                    <ul class="input_select_list">
                                    '.self::get_range_input_select_options($guests_min, $guests_max, 1, $selected_value).'
                                    </ul>
                                    <i class="fas fa-chevron-down"></i>
                                 </div>  
                            </div>
               ...
               } ///// end if isset($av_cal_ages[$age_term['age_id']])
...
////////////////////////////
    ...
    public static function booking_form($booking_obj_id = 0){
        ...
        if ($rules_cat['rules']['basic_booking_period'] == 'day'){
            ...
            $input_time_from = '<div id="booking_time_from_block" class="booking-time-block">
                   '.self::input_select_field('booking_time_from', '', $time_select_arr, (isset($_GET['time_from']) ? $_GET['time_from'] : false)).'
                   </div>';
            ...
            $input_time_to = '<div id="booking_time_to_block" class="booking-time-block">
                   '.self::input_select_field('booking_time_to', '', $time_select_arr, (isset($_GET['time_to']) ? $_GET['time_to'] : false)).'
                   </div>';
        }
        ...
        if ($rules_cat['rules']['basic_booking_period'] == 'single_custom'){
            ...
            <div class="booking-date">
	            ...
	            $input_time_from = '<div id="booking_time_from_block" class="booking-hourly-time-cloud">
                   '.self::input_select_cloud('booking_time_from', $time_select_arr, (isset($_GET['time_from']) && BABE_Calendar_functions::isValidTime($_GET['time_from']) ? $_GET['time_from'] : false)).'
		            ...
		            $input_fields['date_from'] = '
            <div class="booking-form-block booking-date-block">
                <label class="booking_form_input_label"><span class="booking_form_step_num">'.$i.'</span>'.apply_filters('babe_booking_form_date_from_label', __('From:', BABE_TEXTDOMAIN)).'</label>
            <div class="booking-date">
            ...
            <div class="booking-form-block booking-time-block">
                <label class="booking_form_input_label"><span class="booking_form_step_num">'.$i.'</span>'.apply_filters('babe_booking_form_time_from_label', __('Time from:', BABE_TEXTDOMAIN)).'</label>
            <div class="booking-date">
                   '.$input_time_from.'
			</div>
            </div>
        ...
        $input_fields['date_to'] = '';
            ...
            if ($rules_cat['rules']['basic_booking_period'] !== 'recurrent_custom' && $rules_cat['rules']['basic_booking_period'] !== 'single_custom' &&  $rules_cat['rules']['basic_booking_period'] !== 'hour'){
                $input_fields['date_to'] = '
                <div class="booking-form-block booking-date-block">
                <label class="booking_form_input_label"><span class="booking_form_step_num">'.$i.'</span>'.apply_filters('babe_booking_form_date_to_label', __('To:', BABE_TEXTDOMAIN)).'</label>
            <div class="booking-date">
                   <i class="far fa-calendar-alt"></i>
	            ...
	            $input_fields['date_to'] = '
            <div class="booking-form-block booking-date-block">
                <label class="booking_form_input_label"><span class="booking_form_step_num">'.$i.'</span>'.apply_filters('babe_booking_form_date_from_label', __('To:', BABE_TEXTDOMAIN)).'</label>
            <div class="booking-date">
                   <i class="far fa-calendar-alt"></i>
				   <input type="text" class="booking_date" id="booking_date_to" name="booking_date_to" value="'.$date_to.'" placeholder="'.apply_filters('babe_booking_form_date_to_placeholder', '').'" data-post-id="'.$post_id.'" autocomplete="off">
			</div>
            ...
            <div class="booking-form-block booking-time-block">
                <label class="booking_form_input_label"><span class="booking_form_step_num">'.$i.'</span>'.apply_filters('babe_booking_form_time_from_label', __('Time to:', BABE_TEXTDOMAIN)).'</label>
            <div class="booking-date">
                   '.$input_time_to.'
			</div>
            </div>
        ...
        if ($check_add_av_times){
                ...
                <label class="booking_form_input_label"><span class="booking_form_step_num">'.$i.'</span>'.apply_filters('babe_booking_form_time_label', __('Time:', BABE_TEXTDOMAIN)).'</label>
        ...
        if (!isset($rules_cat['category_meta']['categories_remove_guests']) || !$rules_cat['category_meta']['categories_remove_guests']){
            ...
            if ( !empty($guests) && $date_from && $rules_cat['rules']['basic_booking_period'] != 'recurrent_custom' ){
                ...
                $guests_output = __('please, select date first', BABE_TEXTDOMAIN);

            }

            $guests_title = $rules_cat['rules']['booking_mode'] == 'tickets' ? __('Tickets:', BABE_TEXTDOMAIN) : __('Guests:', BABE_TEXTDOMAIN);
        ...
        if (BABE_Settings::$settings['mpoints_active'] && !empty($babe_post) && isset($babe_post['meeting_points']) && isset($babe_post['meeting_place']) && $babe_post['meeting_place'] == 'point'){
            ...
            if (!empty($meeting_points)){
                ...
                $find_closes_loc_text = BABE_Settings::$settings['google_map_remove'] || is_admin() ? '' : ' (' . '<a href="#block_meeting_points">'.__('find closest location', BABE_TEXTDOMAIN).'</a>'.')';
              ...
              <label class="booking_form_input_label"><span class="booking_form_step_num">'.$i.'</span>'.__('Select meeting point', BABE_TEXTDOMAIN). $find_closes_loc_text .':</label>
            ...
            <div id="block_address_map_with_direction" class="babe_overlay_inner">
              <span id="modal_close"><i class="fas fa-times"></i></span>
                ...
                <h3>'.__('Find a route from your location', BABE_TEXTDOMAIN).'</h3>
              
                <div id="google_map_address_with_direction" data-obj-id="" data-lat="" data-lng="" data-address="">
                </div>
                
                <div id="route_to_buttons">
                    <button id="route_to_button_ok" data-point-id="" class="btn button route_to_point_button">'.__('Ok', BABE_TEXTDOMAIN).'</button>
                </div>  
        ...
        if (BABE_Settings::$settings['services_to_booking_form']){
            ...
            if ($services_html){
              ...
              <div class="booking-form-block booking-services-block">
               <label class="booking_form_input_label">'.apply_filters('babe_booking_form_services_label', __('Add Extra', BABE_TEXTDOMAIN)).'</label>
        ...
        $output .= '<form id="booking_form" name="booking_form" method="post" action="" data-post-id="'.$post_id.'" class="booking_form_type_'.$rules_cat['rules']['basic_booking_period'].'">
            ...
            <div id="total_group">
                <label class="booking_form_input_label">'.__('Total:', BABE_TEXTDOMAIN).'</label>
                <div id="booking_form_total">
                </div>
            </div>
            
            <div id="error_group">
                <label class="booking_form_error_label">'.__('Please fill in all the data.', BABE_TEXTDOMAIN).'</label>
            </div>
            ...
            <div class="submit_group">
               ...
               <button class="btn button booking_form_submit" data-post-id="'.$post_id.'"><i class="fas fa-shopping-cart"></i> '.apply_filters('babe_booking_form_submit_button_label', __('Book Now', BABE_TEXTDOMAIN)).'</button>
               
            </div>
            
            </form>';

        /*<button class="btn button booking_form_calculate" data-post-id="'.$post_id.'"><i class="fas fa-calculator"></i>'.__('Calculate', BABE_TEXTDOMAIN).'</button>*/
	 ...
	 * @return string
	 */
	public static function input_select_field_with_order( $field_name, $field_title = '', $values_arr = array(), $selected_key = false, $required = false, $classes = '', $attr = '' ) {
		...
		if (isset($_GET["search_results_sort_by"]) && !empty($_GET["search_results_sort_by"])){
			$sort = (array_search('asc', explode('_', $_GET["search_results_sort_by"])))? 'asc' : 'desc';
		} else {
			$sort = 'asc';
		}
		...
		foreach ($values_arr as $val_id => $value){
			$values_arr[$val_id] = explode(' ', $value)[0];
			if (!array_search( $sort, explode('_', $val_id))){
		...
		switch (explode('_', $selected_key)[0]){
			case 'title':    $icon_name =  ($sort === 'asc') ? '<i class="fa fa-sort-alpha-down"></i>' : '<i class="fa fa-sort-alpha-up"></i>' ; break;
			case 'price':   $icon_name =  ($sort === 'asc') ? '<i class="fa fa-sort-numeric-down"></i>' : '<i class="fa fa-sort-numeric-up"></i>'; break;
			default:        $icon_name =  ($sort === 'asc') ? '<i class="fa fa-sort-amount-down"></i>' : '<i class="fa fa-sort-amount-up"></i>'; break;
		}
	 ...
	 * @return string
	 */
	public static function input_select_cloud( $field_name, $values_arr = [], $selected_key = false, $required = true, $classes = '', $attr = '' ) {
		...
		$output = '
						<div class="input_select_cloud input_select_field_'.esc_attr($field_name_class).' '.esc_attr($classes).'" data-name="'.$field_name.'" '.$attr.' tabindex="0">
...
////////////////////////////
     ...
     * @return array
	 */
    public static function get_time_select_arr( $date, $post_id, $is_from = true, $start_time = '00:00', $order_item_args = [], $step = 60, $is_hourly = false ){
        ...
        $times = $av_cal[$date_from_obj->format('Y-m-d')]['times'];
        $min_booking_period = $av_cal[$date_from_obj->format('Y-m-d')]['min_booking_period'];
        $max_booking_period = $av_cal[$date_from_obj->format('Y-m-d')]['max_booking_period'];
        ...
        if ($is_hourly && ! $is_from &&  $start_time !== '00:00'){
        ...
        $times_count = 0;
        $time_before = '';
        $allow_first_switch = false;

        $min_booking_time_obj = clone $start_time_obj;

        if ($is_from && $min_booking_period){
            if ($is_hourly){
                $min_booking_time_obj->modify('+'.( (int)$min_booking_period ).' hours' );
            } else {
                $min_booking_time_obj = new DateTime($last_time);
            }
        }

        if ( !$is_from && $max_booking_period && $is_hourly){
            $max_booking_time_obj = clone $start_time_obj;
            $max_booking_time_obj->modify('+'.( (int)$max_booking_period ).' hours' );
        } else {
            $max_booking_time_obj = new DateTime('23:59');
        }
        ...
        for($i=0; $i <= 23; $i++){
            ...
            foreach ($min_arr as $min_text){
                ...
                if ($is_from && $min_booking_period){
                    $check_time = $min_booking_time_obj->format('H:i');
                }
                ...
                if( isset($times[$check_time]) && $current_time == $check_time ){
                    ...
                    if (!$is_hourly && !$allow_time && $times_count == 1 && $time_before){
                        unset($output[$time_before]);
                    }
                    $allow_first_switch = true;
                    if (!$is_hourly){
                        $times_count = 0;
                        $time_before = '';
                    } elseif ( $added_times && !$is_from && !$allow_time ) {
                        $max_booking_time_obj = new DateTime($current_time);
                    }
                }

                if ( !$is_hourly && $allow_time && $current_time_obj >= $start_time_obj){
                ...
                if ($is_hourly){
                    ...
                    if ( $current_time_obj < $start_time_obj ){
                        ...
                        $output[$current_time] = false;

                    } elseif ( $allow_time && $current_time_obj <= $max_booking_time_obj ) {

                        if ( $allow_first_switch && $time_before && $is_from){
                            $output[$time_before] = true;
                        }
                        $output[$current_time] = true;
                        $added_times = true;

                    } elseif ($current_time_obj <= $max_booking_time_obj) {

                        $output[$current_time] = false;

                        if ( $allow_first_switch && !$is_from && ( !empty($output[$time_before]) || $start_time_obj == $current_time_obj ) ){
                            $output[$current_time] = true;
                            $added_times = true;
                        }
                    } else {
...
////////////////////////////
    ...
    public static function admin_order_confirm_page_content($args){
        ...
        if ('av_confirmation' != $args['order_status']){
            ...
            $output .= $args['order_status'] != 'not_available' ? sprintf(__( 'Order #%s is confirmed.', BABE_TEXTDOMAIN ), $args['order_num']) : sprintf(__( 'Order #%s is rejected.', BABE_TEXTDOMAIN ), $args['order_num']);
            ...
            $message = $args['action_update'] == 'confirm' ? __( 'Confirm Order #', BABE_TEXTDOMAIN ) : __( 'Reject Order #', BABE_TEXTDOMAIN );
        ...
        if ($order_id){
            ...
            foreach($order_items_arr as $item_id => $item){
                ...
                if ($rules_cat['rules']['basic_booking_period'] != 'recurrent_custom'){
                    ...
                    $date_from_html = '<span class="order_item_td_label">'.__( 'From:', BABE_TEXTDOMAIN ).'</span>'. $date_from_html . '<span class="order_item_td_label">'.__( 'To:', BABE_TEXTDOMAIN ).'</span>
                  <span class="order_item_td_value">'.( date_i18n( $date_format, strtotime($date_to_obj->format('Y-m-d H:i')) ) ).'</span>';
                } else {
                    $date_from_html = '<span class="order_item_td_label">'.__( 'Date:', BABE_TEXTDOMAIN ).'</span>'. $date_from_html . '<span class="order_item_td_label">'.__( 'Time:', BABE_TEXTDOMAIN ).'</span>
                ...
                $duration = $duration ? '
                  ...
                  <span class="order_item_td_label">'.__( 'Duration:', BABE_TEXTDOMAIN ).'</span>
                ...
                if (!empty($meeting_points_arr) && $meeting_point_id && isset($meeting_points_arr[$meeting_point_id])){
                  ...
                  <span class="order_item_td_label">'.__( 'Departure from:', BABE_TEXTDOMAIN ).'</span>
                  ...
                  <span class="order_item_td_label">'.__( 'Departure time:', BABE_TEXTDOMAIN ).'</span>
                ...
                $label_guests = $rules_cat['rules']['booking_mode'] == 'tickets' ? __( 'Tickets:', BABE_TEXTDOMAIN ) : __( 'Guests:', BABE_TEXTDOMAIN );
                
                $guests_html = '
                  <tr><td class="order_item_info order_item_info_guests">
                  <span class="order_item_td_label">'.$label_guests.'</span>
                  <span class="order_item_td_value"><table class="order_item_age_prices" cellpadding="0" cellspacing="0"><tbody>';
                  
                  foreach($item['meta']['guests'] as $age_id => $guests_num){
                    
                     if ($guests_num){
                    
                     $age_title = isset($ages_arr_ordered_by_id[$age_id]) ? $ages_arr_ordered_by_id[$age_id]['name'] : ( $guests_num > 1 ? __( 'persons', BABE_TEXTDOMAIN ) : __( 'person', BABE_TEXTDOMAIN ) );
                     
                     $guests_price_html = isset($item['meta']['price_arr']['clear'][$age_id]) ? '
                      <td><span class="order_item_age_price">'.BABE_Currency::get_currency_price($item['meta']['price_arr']['clear'][$age_id]).'</span></td>' : '';
                ...
                /// Total item price                
                $discount_note = $item['meta']['price_arr']['discount'] ? '<tr><td class="order_item_discount_note">'.sprintf(__( 'Price Cut %d%% off applied', BABE_TEXTDOMAIN ), $item['meta']['price_arr']['discount']).'</td></tr>' : '';
                ...
                <tr><td class="order_item_total_price">'.BABE_Currency::get_currency_price($guests_price).'
                </td></tr>
                '.$discount_note.'
                </tbody></table>
                </td></tr>';
               ...
               <td class="order_item_row_price">
                <table class="table_order_item_total_price"><tbody>
                <tr><td class="order_item_total_price">'.BABE_Currency::get_currency_price($price).'
                </td></tr>
                </tbody></table>
               </td>
               </tr>';
            ...
            <span id="order_items_row_subtotal_amount">'.BABE_Currency::get_currency_price($fee_amount).'</span>
            </td></tr>';
                    }
                }
            ...
            <tr>
            <td class="order_items_row_total order_items_row_subtotal" colspan="2">
            <span class="order_items_row_total_label">'.__( 'Subtotal:', BABE_TEXTDOMAIN ).'</span>
            </td>
            <td class="order_items_row_total_amount order_items_row_subtotal">
            <span id="order_items_row_subtotal_amount">'.BABE_Currency::get_currency_price($subtotal).'</span>
            </td></tr>';
            
            $taxes_percents = (float)apply_filters('babe_html_order_items_post_tax', BABE_Post_types::get_post_tax($post['ID']), $post['ID']);

            if ($taxes_percents){
                $taxes_title = !empty($rules_cat['category_meta']['categories_tax_title']) ? $rules_cat['category_meta']['categories_tax_title'] : __( 'Taxes', BABE_TEXTDOMAIN );
            ...
            <span id="order_items_row_taxes_amount">'.BABE_Currency::get_currency_price($taxes_amount).'</span>
            </td></tr>';
            ...
            <span class="order_items_row_total_label">'.__( 'Total:', BABE_TEXTDOMAIN ).'</span>
            </td>
            <td class="order_items_row_total_amount">
            <span id="order_items_row_total_amount">'.BABE_Currency::get_currency_price($total).'</span>
            </td></tr>';
            ...
            <span class="order_items_row_total_label">'.__( 'Discount coupon', BABE_TEXTDOMAIN ).' '.$order_coupon_num.':</span>
            </td>
            <td class="order_items_row_total_amount order_items_row_coupon">
            <span id="order_items_row_total_amount">-'.BABE_Currency::get_currency_price($order_coupon_amount).'</span>
            </td></tr>';
            }
            ...
            <span class="order_items_row_total_label">'.__( 'Amount Paid:', BABE_TEXTDOMAIN ).'</span>
            </td>
            <td class="order_items_row_total_amount order_items_row_paid">
            <span id="order_items_row_paid_amount">'.BABE_Currency::get_currency_price($prepaid_received).'</span>
            </td></tr>';
            ...
            <span class="order_items_row_total_label">'.__( 'Amount Due:', BABE_TEXTDOMAIN ).'</span>
            </td>
            <td class="order_items_row_total_amount order_items_row_due">
            <span id="order_items_row_paid_amount">'.BABE_Currency::get_currency_price($amount_to_pay).'</span>
            </td></tr>';
    ...
    public static function ajax_apply_coupon_to_order(){
        ...
        if (isset($_POST['nonce']) && wp_verify_nonce( $_POST['nonce'], self::$nonce_title ) && BABE_Coupons::coupons_active()){
           ...
           if (BABE_Order::is_order_valid($order_id, $args['order_num'], $args['order_hash'])){
               $output = BABE_Coupons::update_order_amount($order_id, $coupon_num);
           }  
        }
	 ...
	 */
     public static function coupon_field_to_order_items($content, $order_id, $order_items_arr, $ages_arr){
        ...
        if ($coupon){
            ...
            $coupon_amount = BABE_Coupons::get_coupon_amount($coupon->ID, true);
            ...
            <div class="coupon-form-block-applied">
                '.sprintf(__( 'Coupon Code %s with amount %s was applied', BABE_TEXTDOMAIN ), $coupon_num, $val).'
            </div>';
        }
     ...
     public static function coupon_field_to_checkout_form($content, $args){
        ...
        if (!$coupon){
            ...
            <div class="coupon-form-block">
                <label class="coupon_form_input_label">'.__( 'Enter Coupon Code and get a discount!', BABE_TEXTDOMAIN ).'</label>
            ...
            <span id="coupon_form_submit" class="btn button button-primary">'.__( 'Apply Coupon', BABE_TEXTDOMAIN ).'</span>
            </div>';
...
////////checkout_field_label///
    ...
    public static function checkout_field_label($field_name){
        ...
        switch($field_name){
            case 'first_name':
              $output = __('First name', BABE_TEXTDOMAIN);
              break;
            case 'last_name':
              $output = __('Last name', BABE_TEXTDOMAIN);
              break;
            case 'email':
              $output = __('Email', BABE_TEXTDOMAIN);
              break;
            case 'email_check':
              $output = __('Re-type Email', BABE_TEXTDOMAIN);
              break;
            case 'phone':
              $output = __('Contact Phone', BABE_TEXTDOMAIN);
              break;
            case 'guest':
                $output = __('Guest', BABE_TEXTDOMAIN);
                break;
            case 'extra_guest':
                $output = __('Extra guest', BABE_TEXTDOMAIN);
                break;
	        case 'extra_guests':
		        $output = __('Extra guests', BABE_TEXTDOMAIN);
		        break;
        }
...
////////////////////////////
    ...
    public static function checkout_form($args){
        ...
        $order_currency = $args['order_currency'] ? $args['order_currency'] : BABE_Currency::get_currency();
        
        $args['meta'] = apply_filters('babe_checkout_args', $args['meta'], $args);
        
        $output .= '<h2>'.sprintf(__('Order #%s', BABE_TEXTDOMAIN), $order_num).'</h2>';
        ...
        $input_fields = apply_filters('babe_checkout_input_fields_arr', $input_fields, $args);
            ...
            if($action == 'to_pay'){
                ...
                $payment_fields = '<div class="amount_group"> 
                <label class="checkout_form_amount_label">'.__('Amount to Pay now:', BABE_TEXTDOMAIN).'</label>
                <div class="checkout_form_pay_total">
                <input type="radio" name="payment[amount_to_pay]" id="order_amount_to_pay_deposit" value="deposit" checked="checked"><label for="order_amount_to_pay_deposit">'.BABE_Currency::get_currency_price($prepaid_amount).' '.($payment_model == 'deposit_full' ? __('(deposit)', BABE_TEXTDOMAIN) : '').'</label>
                </div>
                '.($payment_model == 'deposit_full' ? '
                <div class="checkout_form_pay_total">
                <input type="radio" name="payment[amount_to_pay]" id="order_amount_to_pay_full" value="full"><label for="order_amount_to_pay_full">'.BABE_Currency::get_currency_price($total_amount).' '.__('(full)', BABE_TEXTDOMAIN).'</label>
                </div>
                ' : '').'
            </div>';
                ...
                $payment_fields .= '<h2>'.__('Payment Method', BABE_TEXTDOMAIN).'</h2>
                ...
                .$payment_details.'
                 ...
                 <input type="hidden" name="payment[payment_method]" value="'.$first_method.'">
                 </div>
                 ';
                                 
            } else {
                
                $payment_details_text = sprintf(__('You could pay after the items availability confirmation! <br> Confirmation will be sent to your e-mail. <br> Amount to pay online will be %s.', BABE_TEXTDOMAIN), BABE_Currency::get_currency_price($prepaid_amount));
                $payment_details_text = apply_filters('babe_checkout_payment_details_before_av_check', $payment_details_text, BABE_Currency::get_currency_price($prepaid_amount), $args);
                
                $payment_fields .= '<h2>'.__('Payment details', BABE_TEXTDOMAIN).'</h2>
            ...
            $output .= '<h2>'.__('Contact information', BABE_TEXTDOMAIN).'</h2>';
            ...
            <div class="terms_group">
              ...
              <input type="checkbox" name="payment[terms_check]" id="order_terms_check" value="agree" required="required"><label for="order_terms_check">'.__('I read and agree to the terms & conditions', BABE_TEXTDOMAIN).'</label>
              </div>
            ...
            <div class="submit_group">
               ...
               <button class="btn button checkout_form_submit">'.__('Complete My Order', BABE_TEXTDOMAIN).'</button>
    ...
    public static function confirm_content($args){
        ...
        if ($args['order_status'] == 'payment_expected'){
            $output .= '<div class="babe_order_confirm">
              <a href="'.BABE_Order::get_order_payment_page($order_id).'" class="babe_button_order babe_button_order_to_pay">'.__('Pay Now!', BABE_TEXTDOMAIN).'</a>
            </div>';
        }
        
        $output .= '<h4>'.sprintf(__('Order #%s', BABE_TEXTDOMAIN), $args['order_num']).'</h4>';
...
////////////////////////////
    ...
    public static function get_calendar_month_html($date, $av_cal, $discount_arr, $rules_cat, $block_class = ''){
        ...
        $multiplier = (100 - $discount_arr['discount'])/100;
        ...
        $tax_am = (float)apply_filters('babe_html_calendar_month_html_post_tax', BABE_Post_types::get_post_tax($rules_cat['post_id']), $rules_cat['post_id'])/100;
            ...
            $date_obj_current = new DateTime($date_current);
           // $date_selected = $date_obj_default->format('Y-m-d');
            ...
            foreach($daterange as $date_cal){ //// loop by date
                ...
                ///////////////////////
                $add_cell_class = $date_cal->format('m') != $date_obj_month_start->format('m') ? ' cal-cell-empty' : '';
                ...
                if ( isset($av_cal[$date_cal->format('Y-m-d')]['times']) && !empty($av_cal[$date_cal->format('Y-m-d')]['times']) ){
                    ...
                    $min_booking_period = $av_cal[$date_cal->format('Y-m-d')]['min_booking_period'];
                }
                                
                $prev_av_guest = isset($av_cal[$prev_date_obj->format('Y-m-d')]['times']) && !empty($av_cal[$prev_date_obj->format('Y-m-d')]['times']) ? max($av_cal[$prev_date_obj->format('Y-m-d')]['times']) : 0;
                ...
                ($rules_cat['rules']['basic_booking_period'] == 'day' && $av_guest && $av_cal[$date_cal->format('Y-m-d')]['times']['00:00'] && !$last_av_guests)
                   ...
                   && $av_guest && $av_cal[$date_cal->format('Y-m-d')]['times']['00:00'] && $last_av_guests && !$av_guest_min
                ) { 
                ...
                if ( 
                ( $rules_cat['rules']['basic_booking_period'] == 'night' && $av_guest && !$prev_av_guest )
                || ( $rules_cat['rules']['basic_booking_period'] == 'day' && $av_guest && !$av_cal[$date_cal->format('Y-m-d')]['times']['00:00'] )
                ){
                ...
                if ( $date_obj_current > $date_cal || !$av_guest ){
                 ...
                 if ( !empty($rate_current) ){
                    ...
                    $price_from = $price_from*$multiplier;                    
                    $price_from = $price_from + round($price_from * $tax_am, 2); 
                    
                    //// get prices table from $rate_current
                    $prices_table = '
                  <div class="view-rate-details">
                    <div class="view-rate-details-item">
                       <h4>'.$rate_current['rate_title'].'</h4>';
                            
                    $prices_table .= '
                    <div class="rate_min_max">';
                    if ( $rate_current['min_booking_period'] ){
                        $prices_table .= '
                        <span class="rate_details_label">'.__('Minimum booking:', BABE_TEXTDOMAIN).'</span> <span class="rate_details_value">'.$rate_current['min_booking_period'].' '.$units.'</span>
                        ';
                    }
                    if ( $rate_current['max_booking_period'] ){
                        $prices_table .= '
                        <span class="rate_details_label">'.__('Maximum booking:', BABE_TEXTDOMAIN).'</span> <span class="rate_details_value">'.$rate_current['max_booking_period'].' '.$units.'</span>
                        ';
                    }
                    $prices_table .= '
                    </div>';
                    
                    if ( !empty($rate_current['price_general']) ){
                    $prices_table .= '
                    <div class="rate_price_general">
                      <span class="rate_details_label">'.__('General price:', BABE_TEXTDOMAIN).'</span> ';
                    
                    $tmp_prices = array();
                    foreach($rate_current['price_general'] as $age_id => $price){
                       ...
                       $price = $price + round($price * $tax_am, 2);
                    ...
                    if ( !empty($rate_current['prices_conditional']) ){
                        $signs = BABE_Prices::get_conditional_signs();
                        $prices_table .= '
                    <div class="rate_prices_conditional">
                      <h4 class="rate_details_label">'.__('Options', BABE_TEXTDOMAIN).'</h4>
                      <ul class="rate_prices_conditional_details">';
                      
                        foreach($rate_current['prices_conditional'] as $price_conditional){
                            ...
                            $prices_table .= $tmp_output.' <span class="prices_conditional_then">'.__('Price', BABE_TEXTDOMAIN).'</span> ';
                    ...
                    <div class="cal-cell-date">'.$date_cal->format('j').'</div>
                    <div class="cal-cell-pricefrom">'.BABE_Currency::get_currency_price($price_from).'</div>
                    '.$prices_table.'
                    </div>';                 
...
////////////////////////////
    ...
    public static function list_fees( $post, $selected_arr = [] ){
        ...
        foreach($fees_arr as $fee){
            ...
            if ( $general_price ){
                $sr_block .= $is_percent ? $general_price.__( '%', BABE_TEXTDOMAIN ) : BABE_Currency::get_currency_price($general_price);
            } else {
                $sr_block .= __( 'free', BABE_TEXTDOMAIN );
            }
            ...
            if ( !$general_price || $fee['is_mandatory'] ){
                ...
                $mandatory_fields[] = apply_filters('babe_list_fees_block_html', $sr_block, $fee);
            } else {

                $optional_fields[] = apply_filters('babe_list_fees_block_html', $sr_block, $fee);
            }
        ...
        if ( !empty($mandatory_fields) ){
            $output .= '
              <label class="booking_form_input_label">'.apply_filters('babe_booking_form_included_fees_label', __('Mandatory fees', BABE_TEXTDOMAIN)).'</label>
        ...
        if ( !empty($optional_fields) ){
            $output .= '
              <label class="booking_form_input_label">'.apply_filters('babe_booking_form_optional_fees_label', __('Optional fees', BABE_TEXTDOMAIN)).'</label>
              <div id="list_fees_optional">
                '.implode('', $optional_fields).'
              </div>
              ';
        }
...
////////////////////////////
     ...
     * @return string
	 */
    public static function list_add_services($post, $selected_services_arr = []){
        ...
        return self::list_add_services_render_html($post['ID'], 'list_services', $services_arr, $selected_services_arr);
    }
...
////////////////////////////
     ...
     * @return string
     */
    public static function list_add_services_render_html($post_id, $html_id, $services_arr, $selected_services_arr = []){
        ...
        foreach($services_arr as $service){
            ...
            $sr_block .= '<div class="list_service">
                    <div class="list_service_title">
                    <label>
                      <input type="checkbox" name="booking_services[]" value="'.$service['ID'].'" '.$checked.'/>
                      '.__( 'Add ', BABE_TEXTDOMAIN ).'
                    </label>
            ...
            if (!in_array($before_service_type, array('booking', 'day', 'night'))){
                ...
                foreach($ages as $age){
                    ...
                    if (!$is_percent){
                        ...
                        if ($price_value !== ''){
                            ...
                            $price_value_html = $price_value == 0 ? __( 'included', BABE_TEXTDOMAIN ) : BABE_Currency::get_currency_price($price_value + round($price_value * $tax_am, 2));
                        ...
                        if ( !$is_other_prices ){
                            ...
                            $sr_block .= '<span class="service_price_line"><label>'.apply_filters('translate_text', $age['name']).':</label>'.$general_price.' '.__( '%/booking ', BABE_TEXTDOMAIN ).'</span>';

                        } elseif ( isset($other_prices[$age['age_id']]) && $other_prices[$age['age_id']] !== '' ){

                            $sr_block .= '<span class="service_price_line"><label>'.apply_filters('translate_text', $age['name']).':</label>'.$other_prices[$age['age_id']].' '.__( '%/booking ', BABE_TEXTDOMAIN ).'</span>';
                ...
                if (!$is_percent){
                    ...
                    $sr_block .= $general_price == 0 ? __( 'included', BABE_TEXTDOMAIN ) : BABE_Currency::get_currency_price($general_price + round($general_price * $tax_am, 2));

                } else {

                    $sr_block .= $general_price.' '.__( '%/booking ', BABE_TEXTDOMAIN );
    ...
    public static function block_add_services($post){
        ...
        if (!empty($post) && isset($post['services']) && !empty($post['services'])){
             ...
             if (!empty($services_arr)){
                ...
                $ages = empty($ages) ? array(0 => array('name' => __( 'Price', BABE_TEXTDOMAIN ), 'description' => '')) : $ages;
                ...
                foreach($services_arr as $service){
                    ...
                    if ($before_service_type != $service['service_type']){
                        ...
                        '.__( 'Prices ', BABE_TEXTDOMAIN ).BABE_Post_types::$service_type_names[$service['service_type']].'
                        </h4>
                        </div>';
                    ...
                    if (!in_array($before_service_type, array('booking', 'day', 'night'))){
                     ...
                     foreach($ages as $age){
                        ...
                        if (!$is_percent){
                          $sr_block .= $is_other_prices ? (isset($other_prices[$age['age_id']]) && $other_prices[$age['age_id']] !== '' ? '<label>'.apply_filters('translate_text', $age['name']).':</label>'.BABE_Currency::get_currency_price($other_prices[$age['age_id']] + round($other_prices[$age['age_id']] * $tax_am, 2)) : '') : '<label>'.apply_filters('translate_text', $age['name']).':</label>'.BABE_Currency::get_currency_price($general_price + round($general_price * $tax_am, 2));
                        } else {
                          $sr_block .= $is_other_prices ? (isset($other_prices[$age['age_id']]) && $other_prices[$age['age_id']] !== ''? '<label>'.apply_filters('translate_text', $age['name']).':</label>'.$other_prices[$age['age_id']].' '.__( '%/booking ', BABE_TEXTDOMAIN ) : '') : '<label>'.apply_filters('translate_text', $age['name']).':</label>'.$general_price.' '.__( '%/booking ', BABE_TEXTDOMAIN );  
                        }
                        ...
                        if (!$is_percent){
                          $sr_block .= BABE_Currency::get_currency_price($general_price + round($general_price * $tax_am, 2));
                        } else {
                          $sr_block .= $general_price.' '.__( '%/booking ', BABE_TEXTDOMAIN );  
                        }
              ...
              <button class="btn button services_form_submit">'.__('Continue', BABE_TEXTDOMAIN).' <i class="fas fa-caret-right"></i></button>
              </form>';
                 }
                    ...
                    if (!in_array($before_service_type, array('booking', 'day', 'night'))){
                     ...
                     foreach($ages as $age){
                        ...
                        if (!$is_percent){
                          ...
                          $price_value_html = $general_price == 0 ? __( 'included', BABE_TEXTDOMAIN ) : BABE_Currency::get_currency_price($general_price + round($general_price * $tax_am, 2));
    ...
    public static function block_meeting_points($post){
       ...
       // $output .= print_r( $post, 1);
        ...
        $output .= '<div id="block_meeting_points">
            <h3>'.__('Find closest meeting point', BABE_TEXTDOMAIN).'</h3>
            <div class="meeting-points">
              <div class="meeting_points_search">
              <input class="address-autocomplete" name="autocomplete" placeholder="'.__( 'Enter your address', BABE_TEXTDOMAIN ).'" type="text" />
              <button class="btn button find_points">'.__('Search', BABE_TEXTDOMAIN).'</button>
              <div id="travel_mode_panel">
              <label for="travel_mode">'.__('Mode of Travel: ', BABE_TEXTDOMAIN).'</label>
