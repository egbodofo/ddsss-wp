...
//////////////////////////////
	...
	public static function db_upgrade() {
        ...
        if ( !$babe_db_version || version_compare( $babe_db_version, '1.3.0', '<' ) ){
            ...
            if( $wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}babe_rates'") == $wpdb->prefix.'babe_rates' && !get_option('BABE_db_upgrading') ){
                ...
                $wpdb->hide_errors();
                //Drop old columns
                $wpdb->query( "ALTER TABLE {$wpdb->prefix}babe_rates DROP COLUMN rate_type, DROP COLUMN weekend_days, DROP COLUMN min_booking_period, DROP COLUMN color" );
                //Add new columns
                $wpdb->query( "ALTER TABLE {$wpdb->prefix}babe_rates ADD apply_days longtext DEFAULT NULL, ADD start_days longtext DEFAULT NULL, ADD min_booking_period int(4) DEFAULT '0', ADD max_booking_period int(4) DEFAULT '0', ADD price_from float DEFAULT '0', ADD price_general longtext DEFAULT NULL, ADD prices_conditional longtext DEFAULT NULL, ADD color varchar(16) DEFAULT NULL, ADD rate_order int(11) DEFAULT '0', ADD KEY price_from (price_from), ADD KEY rate_order (rate_order)" );
                update_option('BABE_db_version', '1.3.0');
                //error_log('DB upgraded to: 1.3.0');

                /// data migration
                $rates = $wpdb->get_results("
            SELECT * FROM {$wpdb->prefix}babe_rates t_rate
            
            LEFT JOIN
            (
            SELECT rate_id AS meta_rate_id, price_type, age_id, price AS meta_price, price_from AS meta_price_from
            FROM {$wpdb->prefix}babe_rates_meta
            WHERE price_type = 'a_base'
            ) t_rate_meta ON t_rate.rate_id = t_rate_meta.meta_rate_id
            
            ORDER BY rate_id ASC, age_id ASC
            ", ARRAY_A);
                ...
                if (!empty($rates)){
                    ...
                    foreach($week_days as $day_num => $day_name){
                        $week_days[$day_num] = $day_num;
                    }
                    $week_days_serialized = serialize($week_days);

                    $sql = "INSERT INTO {$wpdb->prefix}babe_rates (rate_id,apply_days,start_days,price_from,price_general) VALUES ";

                    $rows = array();
                    foreach ($rates as $rate){
                        $rows[$rate['rate_id']]['price_general'][$rate['age_id']] = floatval($rate['meta_price']);
                        if ($rate['age_id'] == 0 || $rate['age_id'] == $main_age_id){
                            $rows[$rate['rate_id']]['price_from'] = floatval($rate['meta_price']);
                        }
                        $rows[$rate['rate_id']]['apply_days'] = $week_days_serialized;
                        $rows[$rate['rate_id']]['start_days'] = $week_days_serialized;
                    }
                    $start_loop = true;
                    foreach ($rows as $rate_id => $row){
                        ...
                        $sql .= "(".$rate_id.",'".$row['apply_days']."','".$row['start_days']."',".$row['price_from'].",'".serialize($row['price_general'])."')";
                    }

                    $sql .= " ON DUPLICATE KEY UPDATE apply_days=VALUES(apply_days),start_days=VALUES(start_days),price_from=VALUES(price_from),price_general=VALUES(price_general)";
        ...
        if ( version_compare( $babe_db_version, '1.3.20', '<' ) ){
            ...
            if( $wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}babe_rates'") == $wpdb->prefix.'babe_rates' && !get_option('BABE_db_upgrading') ){
                ...
                $wpdb->query( "ALTER TABLE {$wpdb->prefix}babe_rates_meta ADD KEY price_type (price_type), ADD KEY price (price)" );
...
ADD KEY is_default (is_default)" );
                ...
                //error_log('DB upgraded to: 1.3.20');
...
//////////////////////////////
	...
	public static function load_textdomain() {
		$locale = apply_filters( 'plugin_locale', get_locale(), BABE_TEXTDOMAIN );

		load_textdomain( BABE_TEXTDOMAIN, WP_LANG_DIR . '/ba-book-everything/ba-book-everything-' . $locale . '.mo' );
		load_plugin_textdomain( BABE_TEXTDOMAIN, false, BABE_PLUGIN_DIR . '/languages' );
	}
...
//////////////////////////////
    ...
    public static function activation() {
       ...
       BABE_Post_types::register_taxonomies_list();
       
       self::create_roles();

       update_option('BABE_roles_version', BABE_VERSION);
       
       self::create_page('search-result', 'search_result_page', __('Search result', BABE_TEXTDOMAIN));
       self::create_page('checkout', 'checkout_page', __('Checkout', BABE_TEXTDOMAIN));
       self::create_page('add_services', 'services_page', __('Add Services', BABE_TEXTDOMAIN));
       self::create_page('confirmation', 'confirmation_page', __('Confirmation', BABE_TEXTDOMAIN));
       self::create_page('terms-conditions', 'terms_page', __('Terms & Conditions', BABE_TEXTDOMAIN), __('You have to edit "Terms & Conditions" page to replace this start content with your own.', BABE_TEXTDOMAIN));
       self::create_page('my-account', 'my_account_page', __('My Account', BABE_TEXTDOMAIN));
       self::create_page('admin-confirmation', 'admin_confirmation_page', __('Admin confirmation', BABE_TEXTDOMAIN));
...
////////////////////////////
	...
	public static function create_roles() {
		...
		// Customer role
		add_role( 'customer', __( 'Customer', BABE_TEXTDOMAIN ), array(
			'read' 					=> true
		) );

		// Manager role
		add_role( 'manager', __( 'Manager', BABE_TEXTDOMAIN ), array(
			'read'                   => true,
...
CREATE TABLE {$wpdb->prefix}babe_rates_meta (
...
//////////////////////////////
    ...
    /*      'Bike' => array(
        ...
        self::$demo_ages = array(
          'adult' => array(
              'slug' => 'adult',
              'title' => __( 'Adult', BABE_TEXTDOMAIN ),
              'description' => __( '18+ years', BABE_TEXTDOMAIN ),
              'menu_order' => 1,
          ),
          'youth' => array(
              'slug' => 'youth',
              'title' => __( 'Youth', BABE_TEXTDOMAIN ),
              'description' => __( '13-17 years', BABE_TEXTDOMAIN ),
              'menu_order' => 2,
          ),
          'children' => array(
              'slug' => 'children',
              'title' => __( 'Children', BABE_TEXTDOMAIN ),
              'description' => __( '0-12 years', BABE_TEXTDOMAIN ),
              'menu_order' => 3,
          ),
        );
...
///////////////////////////////////////    
    ...
    public static function setup_tax_features(){
        ...
        if ($tax_name_term):
            ...
            //// insert term
            $inserted_term = wp_insert_term(
               __( 'Features', BABE_TEXTDOMAIN ),   // the term 
            ...
            $new_tax_name = __( 'Features', BABE_TEXTDOMAIN );
            ...
            $inserted_term = wp_insert_term(
               __( 'Feature 1', BABE_TEXTDOMAIN ),
               $new_tax_slug
            );
            $inserted_term = wp_insert_term(
               __( 'Feature 2', BABE_TEXTDOMAIN ),
               $new_tax_slug
            );
            $inserted_term = wp_insert_term(
               __( 'Feature 3', BABE_TEXTDOMAIN ),
               $new_tax_slug
            );
       ...
       */
       ...
       if (!empty($demo_rules)){
          foreach ($demo_rules as $demo_rule_title => $demo_rule){
            ...
            update_term_meta($term_id, 'categories_step_title', __( 'Description', BABE_TEXTDOMAIN ));
            update_term_meta($term_id, 'categories_services', 1);
            update_term_meta($term_id, 'categories_services_title', __( 'Services', BABE_TEXTDOMAIN ));
            update_term_meta($term_id, 'categories_faq', 1);
            update_term_meta($term_id, 'categories_faq_title', __( 'Questions & Answers', BABE_TEXTDOMAIN ));
...
///////////////////////////////////////    
    ...
    public static function create_page_all_items(){
        ...
        return self::create_page('all-items', 'all_items_page', __('All items', BABE_TEXTDOMAIN), '[all-items]');
    ...
    public static function setup_posts_faq(){
        ...
        if ( empty($posts) ){
          ...
          $post_id = wp_insert_post(array (
            'post_type' => BABE_Post_types::$faq_post_type,
            'post_title' => __('Do you speak spanish?', BABE_TEXTDOMAIN),
          ...
          $post_id = wp_insert_post(array (
            'post_type' => BABE_Post_types::$faq_post_type,
            'post_title' => __('Question 2?', BABE_TEXTDOMAIN),
            'post_content' => 'Lorem ipsum dolor sit amet, utinam munere antiopam vel ad. Qui eros iusto te. Nec ad feugiat honestatis. Quo illum detraxit an. Ius eius quodsi molestiae at, nostrum definitiones his cu. Discere referrentur mea id, an pri novum possim deterruisset. Eum oratio reprehendunt cu. Nec te quem assum postea.',
            'post_status' => 'publish',
            'comment_status' => 'closed',
            'post_author'   => 1,
...
///////////////////////////////////////    
    ...
    public static function setup_posts_services(){
        ...
        if ( empty($posts) ){
          ...
          $post_id = wp_insert_post(array (
            'post_type' => BABE_Post_types::$service_post_type,
            'post_title' => __('Service per booking', BABE_TEXTDOMAIN),
            'post_content' => 'Lorem ipsum dolor sit amet, utinam munere antiopam vel ad. Qui eros iusto te. Nec ad feugiat honestatis. Quo illum detraxit an. Ius eius quodsi molestiae at, nostrum definitiones his cu. Discere referrentur mea id, an pri novum possim deterruisset. Eum oratio reprehendunt cu. Nec te quem assum postea.',
            'post_status' => 'publish',
            'comment_status' => 'closed',
            'post_author'   => 1,
          ...
          $post_id = wp_insert_post(array (
            'post_type' => BABE_Post_types::$service_post_type,
            'post_title' => __('Service per person', BABE_TEXTDOMAIN),
            'post_content' => 'Lorem ipsum dolor sit amet, utinam munere antiopam vel ad. Qui eros iusto te. Nec ad feugiat honestatis. Quo illum detraxit an. Ius eius quodsi molestiae at, nostrum definitiones his cu. Discere referrentur mea id, an pri novum possim deterruisset. Eum oratio reprehendunt cu. Nec te quem assum postea.',
            'post_status' => 'publish',
            'comment_status' => 'closed',
            'post_author'   => 1,
    ...
    public static function setup_posts_places(){
      ...
      if (BABE_Settings::$settings['mpoints_active']){  
        ...
        if ( empty($posts) ){
          ...
          $address = array(
            'address' => __('Santorini, Greece', BABE_TEXTDOMAIN),
            'latitude' => '36.3931562',
          ...
          $post_id = wp_insert_post(array (
            'post_type' => BABE_Post_types::$mpoints_post_type,
            'post_title' => __('Place 1', BABE_TEXTDOMAIN),
            'post_content' => 'Lorem ipsum dolor sit amet, utinam munere antiopam vel ad. Qui eros iusto te. Nec ad feugiat honestatis. Quo illum detraxit an. Ius eius quodsi molestiae at, nostrum definitiones his cu. Discere referrentur mea id, an pri novum possim deterruisset. Eum oratio reprehendunt cu. Nec te quem assum postea.',
            'post_status' => 'publish',
            'comment_status' => 'closed',
            'post_author'   => 1,
          ...
          $post_id = wp_insert_post(array (
            'post_type' => BABE_Post_types::$mpoints_post_type,
            'post_title' => __('Place 2', BABE_TEXTDOMAIN),
            'post_content' => 'Lorem ipsum dolor sit amet, utinam munere antiopam vel ad. Qui eros iusto te. Nec ad feugiat honestatis. Quo illum detraxit an. Ius eius quodsi molestiae at, nostrum definitiones his cu. Discere referrentur mea id, an pri novum possim deterruisset. Eum oratio reprehendunt cu. Nec te quem assum postea.',
            'post_status' => 'publish',
            'comment_status' => 'closed',
            'post_author'   => 1,
        ...
        foreach ( self::$demo_categories as $term_category_id => $category_title ){
          ...
          $meta_input = array(
              ...
              'images' => array (
                 0 => array (
                    'image_id' => $image_id,
                    'image' => $image_url,
                    'description' => __('Image description (optional)', BABE_TEXTDOMAIN),
                 ),
               ),
               'discount' => array (
                 'discount' => '',
          ...
          if ( ($category_title == 'Tour' || $category_title == 'One time event') && $category_meta['categories_address'] ){
              ...
              $meta_input['address_'.$category_term->slug] = array(
                'address' => __('Bryce Canyon National Park, USA', BABE_TEXTDOMAIN),
          ...
          $post_id = wp_insert_post(array (
            ...
            'post_title' => $category_title.' '.__('example', BABE_TEXTDOMAIN),
            'post_content' => 'Lorem ipsum dolor sit amet, utinam munere antiopam vel ad. Qui eros iusto te. Nec ad feugiat honestatis. Quo illum detraxit an. Ius eius quodsi molestiae at, nostrum definitiones his cu. Discere referrentur mea id, an pri novum possim deterruisset. Eum oratio reprehendunt cu. Nec te quem assum postea.',
            'post_status' => 'publish',
            'post_author'   => 1,
            'menu_order' => $i,
          ...
          $rate_arr = array(
             ...
             '_rate_min_booking' => '',
             '_rate_max_booking' => '',
             '_rate_title' => __('General', BABE_TEXTDOMAIN),
             '_rate_date_from' => '',
             '_rate_date_to' => '',
          );
